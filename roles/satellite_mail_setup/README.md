...
- Generates an installer inventory (`installer_inventory.yml` by default) with optional targets (satellite, aap, openshift).
...
- `satellite_mail_targets`: [satellite, aap, openshift]
...
- Network prompts (overrideable in `env.yml`): `satellite_domain`, `satellite_primary_interface`, optional `satellite_fqdn_ip`, `satellite_gateway`, `satellite_nameserver`, `satellite_dhcp_range_start`/`satellite_dhcp_range_end`, `satellite_pxeserver`.
...
- hosts: satellite
  roles:
    - role: satellite_mail_setup
      vars:
        satellite_mail_targets: ['satellite','aap']
        satellite_fqdn: satellite.poc.example.com
