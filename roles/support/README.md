...
The `support` role orchestrates post-deployment validation, testing, and support tasks. It performs preflight checks, runs test suites, manages backups, and provides deployment diagnostics.

**Key Responsibility**: Validate deployment and provide ongoing support capabilities.
...
```bash
ansible-playbook roles/support/tasks/health_check.yml
```
...
**Generate Report**:
```bash
ansible-playbook roles/support/tasks/deployment_report.yml
```
...
- See integration/* for integration-specific tests
