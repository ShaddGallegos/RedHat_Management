...
Comprehensive Ansible role for configuring dual network interfaces and all platform_provisioning services (DHCP, DNS, PXE, TFTP).

## Overview

This role deploys and configures all platform_provisioning services on `scenario_satellite.prod.spg` server with dual network connectivity:
...
- `eth1` connection (private platform_provisioning)
...
- **Performance**: No contention between platform_provisioning and external traffic
