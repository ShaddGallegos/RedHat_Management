...
if TYPE_CHECKING:
    # allow static type checkers to resolve names without forcing runtime imports
...
class ForemanClient:
    def __init__(self, base_url: str, token: Optional[str] = None,
                 ...
                 verify: bool = True, per_page: int = 500, timeout: int = 30):
        # if requests is not available, raise a clear error at runtime
        if requests is None:
            raise AnsibleParserError(
                "foreman plugin requires the 'requests' package. "
    ...
    def _get_hosts_page(self, page: int = 1, search: Optional[str] = None) -> Dict[str, Any]:
        params: Dict[str, Any] = {"per_page": self.per_page, "page": page}
        if search:
            params["search"] = search
        url = f"{self.base}/api/hosts"
        r = self.session.get(url, params=params, verify=self.verify, timeout=self.timeout)
        r.raise_for_status()
        return r.json()
    ...
    def list_hosts(self, search: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            if isinstance(data, dict) and "results" in data:
                ...
                # ensure batch is a sized sequence (avoid None) so len() is safe for type checkers
                batch = (data.get("results") if isinstance(data, dict) else []) or []
                total = len(batch)
            if not batch:
                break
            hosts.extend(batch)
...
def normalize_host_item(item: Dict[str, Any], host_field: str) -> Optional[Dict[str, Any]]:
    ...
    if '.' in host_field:
        ...
        hostname = item.get(host_field) or item.get('name') or item.get('host') or item.get('fqdn')
    if not hostname:
        return None
    # Best-effort extract IP and some metadata
    ip = item.get('ip') or item.get('ip_address') or item.get('primary_interface', {}).get('ip') if isinstance(item.get('primary_interface'), dict) else None
...
class InventoryModule(BaseInventoryPlugin):
    ...
    def parse(self, loader, path, cache=True):
        ...
        client = ForemanClient(base_url=url, token=token, user=user, password=password, verify=verify, per_page=per_page)
        try:
...
def cli_main():
    import argparse
    parser = argparse.ArgumentParser(description="Foreman/Satellite dynamic inventory CLI")
    ...
    from typing import Dict, Any as _Any  # local import to keep scope clear
    inv: Dict[str, _Any] = {"_meta": {"hostvars": {}}, "all": {"children": ["foreman_hosts"]}, "foreman_hosts": {"hosts": []}}
    for item in hosts:
        norm = normalize_host_item(item, "name")
        if not norm:
            continue
        h = norm["name"]
        hosts_list = cast(List[str], inv["foreman_hosts"]["hosts"])
        hosts_list.append(h)
    ...
    if args.list:
        ...
        hostvars = cast(Dict[str, _Any], inv["_meta"]["hostvars"])
