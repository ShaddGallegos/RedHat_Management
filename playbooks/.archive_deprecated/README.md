...
# 
# This directory contains consolidated orchestration playbooks.
...
- setup_framework.yml → roles/deployment_setup
- generate_inventory.yml → roles/inventory_generator
- deploy_libvirt.yml → roles/infrastructure_prep
- [30+ other utility playbooks] → Various roles

## New Role-Based Architecture

### Orchestration Roles
1. **deployment_setup** - Interactive configuration (formerly setup_framework.yml)
2. **inventory_generator** - Inventory generation (formerly generate_inventory.yml)
3. **infrastructure_prep** - Libvirt setup (formerly deploy_libvirt.yml)
