...
- `roles/aap_deploy` — deployment-related orchestration (playbooks/tasks previously in aap_deployment)
...
1) `roles/aap_2_6_setup/` -> `roles/aap/`
- Copy `roles/aap_2_6_setup/tasks/*` -> `roles/aap/tasks/install.yml` (merge into single install task file)
- Copy `roles/aap_2_6_setup/defaults/main.yml` -> merge into `roles/aap/defaults/main.yml` (normalize var names)
- Copy `roles/aap_2_6_setup/meta/*` -> `roles/aap/meta/`

2) `roles/aap_controller_setup/` -> `roles/aap_controller/`
- Copy `tasks/*` -> `roles/aap_controller/tasks/`
- Copy `defaults/`, `vars/`, `templates/`, `handlers/` -> corresponding `roles/aap_controller/` dirs

3) `roles/aap_deployment/` -> `roles/aap_deploy/`
- Copy `tasks/*` -> `roles/aap_deploy/tasks/`
- Copy `defaults/`, `vars/`, `templates/`, `handlers/` -> `roles/aap_deploy/`

4) `roles/aap_credentials_config/` -> `roles/aap_credentials/`
...
5) `roles/aap_inventories_config/` -> `roles/aap_inventories/`
- Copy `tasks/*`, `defaults/*`, `tests/*` -> `roles/aap_inventories/`

6) `roles/aap_projects_config/` -> `roles/aap_projects/`
- Copy `tasks/*`, `defaults/*`, `tests/*` -> `roles/aap_projects/`

7) `roles/aap_templates_config/` -> `roles/aap_templates/`
- Copy `tasks/*`, `defaults/*`, `tests/*` -> `roles/aap_templates/`

8) `roles/ansible_automation_platform/` (composite) -> `roles/aap`
- The composite imports `install.yml, configure.yml, test.yml, backup.yml, restore.yml`.
- Move `roles/ansible_automation_platform/tasks/*` into `roles/aap/tasks/` with the same filenames (or keep `roles/ansible_automation_platform` as a compatibility wrapper that includes `roles/aap`).

References to update (examples):
- Update playbooks that include roles by name (search `git grep -n "aap_"` and `ansible_automation_platform`) and replace with new role names.
- Update docs (e.g., `docs/products/aap/README.md`) to reference canonical role names.

Variable reconciliation guidance:
- Where both `roles/aap_*` and `ansible_automation_platform` define defaults, create a merged `roles/aap/defaults/main.yml` and preserve legacy names as deprecation aliases, e.g.:
